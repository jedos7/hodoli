...
판정은 완성된 일봉으로만 한다.
  ...
  4. 엄선 조건 (각각 탈락 사유):
     - 눌림 폭    : 고점 대비 -depth_min ~ -depth_max % 사이 (기본 -5 ~ -15)
     - 거래량 감소: 눌리는 동안 평균 거래량 ≤ 급등일 거래량 × vol_ratio_max (기본 0.5)
...
def find_pullbacks(
    ...
    depth_any_min: float = 3.0, depth_min: float = 5.0, depth_max: float = 15.0,
    vol_ratio_max: float = 0.5, ma_tol: float = 0.0, hold_for_ret: int = 3,
) -> list[PullbackSetup]:
    if len(candles) < 30:
        return []
    ...
    for i in range(1, len(candles)):
        ...
        for j in range(i + 2, min(i + lookahead, len(candles) - 1) + 1):
            ...
            out.append(PullbackSetup(day.date, code, name, theme, spike.date, spike_pct, peak.date, peak.high, depth, pull_days,
                                     vol_ratio, ma20_pct, day.close, day.amount / 1e8, reasons, fwd))
    return out
