...
- **켜기**: `D:\theme_radar\start_server.bat` 를 더블클릭. 8000번 포트에 남아 있는 옛 서버를 정리하고 새로 시작합니다. 창을 닫으면 서버가 꺼집니다.
- **끄기**: 그 창에서 Ctrl+C, 또는 `stop_server.bat` 더블클릭.
- 켠 뒤 브라우저에서 `http://127.0.0.1:8000`. 화면 맨 아래 줄에 "실시간 연결 (n종목)" 이 보이면 정상, "실시간 끊김" 이면 키움 접속 문제(IP 등록·키)입니다.
- 코드를 고친 뒤에는 껐다 켜야 반영됩니다 (`--reload` 는 이 환경에서 불안정해 배치 파일에서는 쓰지 않습니다).
...
- 눌림 깊이별로 보면 (급등주 890건) 0~-1% 눌림은 승률 49.3%(동전 던지기, 최근 표본은 평균 0.00%), -1~-3% 는 52.9%, **-3% 이하는 77.4% 평균 +9.2%**. -2% 이하 + 낮에 고가에서 밀린 조합은 65.9% +6.1%. 다음 날 시장이 -1% 넘게 갭다운한 날은 승률 27% 로, 결국 시장이 안 빠지는 날에 거는 규칙입니다.
- 2026-09-10 에 얕은 눌림(-0.2~-1.1%)만 있던 날 3종목을 보냈더니 2종목이 갭 하락(코스피200 -2.8% 갭다운 날). 그 뒤로 **-2% 이하 눌림만 후보**로 좁혔습니다 (`app/nxt_close.py` 의 `MIN_DIP`). 알림 오는 날이 5일에 한 번꼴로 줄지만, 얕은 눌림은 보내 봐야 정보가 없습니다.

그래서 `SCHEDULE_NXT`(기본 19:50)에 서버가 후보(오늘 고가놀이 엄선 + 오늘 +8% 이상·50억 이상·상한가 제외)의 NXT 현재가를 키움에서 읽어
KRX 종가보다 **2% 넘게 내려** 있는 종목만 **종가배팅 후보** 알림으로 냅니다 (`app/nxt_close.py`). 눌림이 얕기만 한 날은 "후보 없음 · 오늘은 쉬는 날" 로 옵니다.

**시장 갭다운 위험 표시** (`app/market_risk.py`, 검증 `scripts/sweep_mkt_risk.py`): 종가배팅은 다음 날 시장이 -1% 넘게 갭다운하면 승률 27% 라, 19:50 에 알 수 있는 정보로 그걸 미리 잴 수 있는지 120거래일로 검증했습니다.
15:30→19:45 나스닥 선물 움직임이 다음 날 전 종목 시가갭과 상관 +0.32(15분봉 +0.40)로 가장 좋았고, 환율·유가·항셍은 예측력이 없었습니다.
- 🟠 **주의** = 나스닥 선물 -0.3% 이하: 과거 18일 중 9일 갭다운(평소 18%), 종가배팅 일평균 +0.9% (평소 +7.4%). 승률은 비슷하고 기대 수익이 줄어드니 수량을 줄이는 날.
- 🔴 **위험** = 나스닥 선물 -0.5% 이하 또는 S&P 선물 -0.3% 이하: 8일 중 5일 갭다운. 쉬거나 최소 수량.
- 🟢 **보통** = 그 외. 2026-09-10 저녁은 나스닥 선물 -0.35% 로 '주의' 였고 다음 날 코스피200 이 -2.8% 갭다운했습니다.
알림 둘째 줄에 `시장 위험 [주의] 15:30 이후 나스닥선물 -0.35% · S&P선물 -0.22% · WTI +1.79% · 환율 +0.05% — …` 로 붙고, `GET /api/nxtclose` 의 `market` 에도 들어갑니다. 미국 정규장(22:30~)은 19:50 뒤라 이 표시가 못 보는 위험이 절반쯤 남습니다. 감시 패널의 🌙 NXT 버튼으로 즉시 점검, `GET /api/nxtclose` 로 목록.
NXT 시세는 키움 종목코드 `_NX` 로 조회하며 키움 피드일 때만 됩니다. 알림은 후보를 알려 줄 뿐 주문하지 않습니다.

## 텔레그램 알림 (app/notify.py)

돌파·이탈·종가배팅 후보 알림을 휴대폰으로 받습니다. 스케줄 작업이 끝나면 한 줄 요약도 옵니다.

1. 텔레그램에서 **BotFather** 를 찾아 `/newbot` → 이름과 아이디(…bot 으로 끝나야 함)를 정하면 토큰이 나옵니다. 이 토큰을 `.env` 의 `TELEGRAM_BOT_TOKEN` 에 넣고 서버를 재시작합니다.
2. 방금 만든 봇을 찾아 아무 말이나 한 번 보냅니다. 그 뒤 브라우저에서 `http://127.0.0.1:8000/api/notify/chatid` 를 열면 채팅 ID 가 보입니다. `.env` 의 `TELEGRAM_CHAT_ID` 에 넣고 다시 재시작합니다.
3. `http://127.0.0.1:8000/docs` 에서 `POST /api/notify/test` 를 실행하면 휴대폰에 확인 메시지가 옵니다. 화면 맨 아래 줄에 "텔레그램 연결됨" 이 뜹니다.

`NOTIFY_KINDS` 로 보낼 종류를 고르고(기본 brk,stop,nxt), `NOTIFY_DAILY=0` 이면 요약은 끕니다. 토큰이 비어 있으면 화면 알림만 옵니다.
...
| `SCHEDULE_NXT` | 19:50 | NXT 야간장 마감 직전 종가배팅 후보 알림 (KRX 종가보다 2% 넘게 내려온 종목만) |
...
## 시장 지수 패널 (app/collectors/market_index.py)

왼쪽 위 '시장 지수' 와 '코스피/코스닥 당일 수급' 은 2026-09-11 까지 처음 시제품의 **고정 숫자**(코스피 7,069 …)에 난수를 섞어 흔든 가짜였습니다. 지금은 실제 값만 보여줍니다.
- 코스피·코스닥·코스피200·코스피200 선물 현재가/등락/거래대금(조)/시가: 네이버 지수 API (키 불필요, 1분 주기, 장 시간 밖에는 10분).
- 상승/하락 종목 수(괄호는 상한/하한), 투자자별 순매수(억): 네이버. 키움 피드면 금투·연기금·투신까지 키움 ka10051 로 채웁니다 (수급 제목 옆에 출처 표시).
- 베이시스 = 선물 − 코스피200 (양수 콘탱고, 음수 백워데이션). ADR 20일 = 최근 20거래일 상승 종목 수 ÷ 하락 종목 수 (스크리너 일봉 캐시 기준이라 증권사 값과 조금 다름).
- 상태 배지: VIX 25 초과 또는 코스피 -2% 이하 → 불안, VIX 20 초과 또는 -1% 이하 → 주의, 그 외 안정.
- 뺀 것: VKOSPI·예탁금·신용잔고·옵션 PCR·미결제약정 (무료 소스가 없어 가짜로 두느니 뺐습니다).
`GET /api/market`, `POST /api/market/refresh`.
...
| 증상 | 원인 | 조치 |
|---|---|---|
| 화면 배지가 노란 "모의 피드" / "실시간 끊김" | 키움 접속 실패 (IP 미등록, 키 오류, 토큰 만료) | 키움 사이트 IP 등록 확인 → `py scripts\probe_kiwoom.py` 대신 `/api/health` 의 realtime 확인 → 서버 재시작 |
| "Address already in use" | 옛 서버가 남아 있음 | `stop_server.bat` 또는 `start_server.bat` (자동 정리) |
| 장중인데 "실시간 체결 n분째 없음" | 웹소켓이 조용히 끊김 | 서버가 60초 안에 재접속을 시도합니다. 5분 넘게 지속되면 재시작 |
| 텔레그램 알림이 안 옴 | 토큰/채팅 ID 미설정, 봇에 /start 안 함 | 화면 맨 아래 텔레그램 상태 확인 → `/api/notify/chatid` → `POST /api/notify/test` |
| 19:50 종가배팅 알림이 "건너뜀" | 오늘 확정 일봉 없음 (휴장일 또는 15:45 스크리너 미실행) | 정상. 휴장일이면 다음 거래일에 |
| 감시 목록이 비어 있음 | 오늘 눌림목 엄선 자리가 없거나 결과 파일이 4일 넘게 오래됨 | 정상. 15:45 결과로 다시 채워짐 |
| 고가놀이·눌림목 결과가 어제 날짜 | 장중이라 오늘 봉 제외 | 정상. 15:45 이후 오늘 포함 |
| 네이버 수집 실패 로그 | 페이지 구조 변경 또는 차단 | 이전 결과 유지. 파서(`app/collectors/*.py`) 수정 필요 |
